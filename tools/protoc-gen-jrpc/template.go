...
{{ range $message := .Messages }}
	...
	func (m *{{ $message.Name }}) UnmarshalJSON(data []byte) error {
		type Alias {{ $message.Name }}
		a := (*Alias)(m)
		if err := json.Unmarshal(data, a); err != nil {
			return err
		}

		{{ range $fieldName := $message.TimeFields }}
			if m.{{ $fieldName }} != "" {
				if _, err := time.Parse(time.RFC3339Nano, m.{{ $fieldName }}); err != nil {
					return err
				}
			}
		{{ end }}
...
{{ range $message := .Messages }}
	{{ range $fieldName := $message.TimeFields }}
		// Get {{ $fieldName }}Time parses {{ $fieldName }} as a time 
		func (m *{{ $message.Name }}) Get{{ $fieldName }}Time() time.Time {
			if m.Get{{ $fieldName }}() == "" {
				return time.Time{}
			}
			...
			t, _ := time.Parse(time.RFC3339Nano, m.Get{{ $fieldName }}())
			return t
		}
	{{ end }}
{{ end -}}
