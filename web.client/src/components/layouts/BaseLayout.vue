<template>
    <div class="base-layout">
        <div class="grid-item">
            <div class="nav-bar">
                <div class="nav-bar__element nav-bar__element--right">
                    <button v-if="done" @click="goBack">Done</button>
                </div>
            </div>
            ...
            <div v-else-if="loading" class="loading">
                <FontAwesomeIcon class="loading__icon" icon="spinner-third" size="4x" spin />
                <h1 class="loading__heading">Loading</h1>
            </div>

            <template v-else>
                <div class="header" :class="{ 'header--icon': icon }">
                    <FontAwesomeIcon class="header__icon" :icon="icon" size="4x" />
                    <h1 class="header__heading"><slot name="heading"></slot></h1>
                </div>
        ...
        <transition :name="childTransition">
            <router-view class="extra-details grid-item"></router-view>
        </transition>
